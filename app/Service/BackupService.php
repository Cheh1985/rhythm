...
final class BackupService
{
    ...
    private const REQUIRED = [
        ...
        'workout_templates'=>['id','code'],'workout_plans'=>['id','external_plan_id'],'workout_exercises'=>['id','workout_plan_id','exercise_id','sequence_no'],
    ...
    public function export(int $userId): array
    {
        ...
        $data = [];
        foreach (self::TABLES as $table) $data[$table] = $this->exportRows($table, $userId);
        return [
            'schema'=>'training-diary-backup','schema_version'=>'1.0','backup_id'=>'backup-'.bin2hex(random_bytes(16)),
    ...
    public function validate(string $json): array
    {
        ...
        if ($backup['schema']!=='training-diary-backup' || $backup['schema_version']!=='1.0') throw new InvalidArgumentException('Формат резервной копии не поддерживается.');
        ...
        if (!is_array($data) || array_is_list($data) || array_diff(array_keys($data),self::TABLES) || array_diff(self::TABLES,array_keys($data))) throw new InvalidArgumentException('Набор секций backup v1.0 неполон или содержит неизвестную секцию.');
        $total=0;
    ...
    {
        $counts=[];$total=0;
        foreach(self::TABLES as $table){$counts[$table]=count($backup['data'][$table]??[]);$total+=$counts[$table];}
    ...
    public function restore(array $backup,int $userId): array
    {
        ...
        return $this->transaction(function(PDO $pdo)use($backup,$userId):array{
            ...
            $data=$backup['data'];$maps=[];$counts=array_fill_keys(self::TABLES,['inserted'=>0,'skipped'=>0]);$counts['training_sequence']['skipped']=count($data['training_sequence']);
            ...
            foreach($data['training_programs'] as $row){$id=$this->id($pdo,'SELECT id FROM training_programs WHERE user_id=? AND external_program_id=?',[$userId,$row['external_program_id']]);$new=$id===null;$id??=$this->insert($pdo,'training_programs',$row,['user_id'=>$userId]);$maps['training_programs'][(string)$row['id']]=$id;$mark('training_programs',$new);}
            $pending=$data['program_versions'];
            while($pending){$progress=false;foreach($pending as $key=>$row){$program=$maps['training_programs'][(string)$row['program_id']]??null;$parent=$row['parent_version_id']===null?null:($maps['program_versions'][(string)$row['parent_version_id']]??null);if($program===null||($row['parent_version_id']!==null&&$parent===null))continue;$id=$this->id($pdo,'SELECT id FROM program_versions WHERE program_id=? AND version_number=?',[$program,$row['version_number']]);$new=$id===null;$id??=$this->insert($pdo,'program_versions',$row,['program_id'=>$program,'parent_version_id'=>$parent]);$maps['program_versions'][(string)$row['id']]=$id;$mark('program_versions',$new);unset($pending[$key]);$progress=true;}if(!$progress)throw new InvalidArgumentException('Backup содержит разорванную цепочку версий.');}
            ...
            foreach($data['workout_exercises'] as $row){$plan=$maps['workout_plans'][(string)$row['workout_plan_id']]??null;if($plan===null||!$this->exercise($pdo,(string)$row['exercise_id'],$userId))throw new InvalidArgumentException('Backup ссылается на недоступное упражнение или план.');$id=$this->id($pdo,'SELECT id FROM workout_exercises WHERE workout_plan_id=? AND sequence_no=?',[$plan,$row['sequence_no']]);$new=$id===null;$id??=$this->insert($pdo,'workout_exercises',$row,['workout_plan_id'=>$plan]);$maps['workout_exercises'][(string)$row['id']]=$id;$mark('workout_exercises',$new);}
