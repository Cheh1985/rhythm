...
<?php foreach ($report['exercises'] as $index => $exercise): $fact = $exercise['fact']; ?>
    <article class="result-detail">
        ...
        <span><?php foreach ($fact['sets'] as $set): if ($set['type'] === 'working'): ?><?= e(\App\Domain\Weight::text($set)) ?> ×<?= (int) $set['reps'] ?> <small>RIR <?= e($set['rir']) ?></small> <?php endif; endforeach; ?></span>
    ...
    <div class="progression-card"><strong>Предложение: <?= e(\App\Domain\Weight::fromKg((float) $suggestion['current_weight_kg'], $session['exercises'][$index]['weight_unit'] ?? 'kg')) ?> → <?= e(\App\Domain\Weight::fromKg((float) $suggestion['suggested_weight_kg'], $session['exercises'][$index]['weight_unit'] ?? 'kg')) ?> <?= e(unit($session['exercises'][$index]['weight_unit'] ?? 'kg')) ?></strong><small><?= e(system_message($suggestion['reason'])) ?> Программа не изменена.</small>
        <?php if ($suggestion['status'] === 'pending'): ?><form method="post" action="<?= e(url('/progression/' . $suggestion['id'])) ?>" class="inline-edit"><input type="hidden" name="_csrf" value="<?= e($csrf) ?>"><input type="hidden" name="session_id" value="<?= (int) $session['id'] ?>"><input type="hidden" name="weight_unit" value="<?= e($session['exercises'][$index]['weight_unit'] ?? 'kg') ?>"><label>Принять вес<input type="number" name="accepted_weight_value" min="0.01" max="2000" step="0.01" value="<?= e(\App\Domain\Weight::fromKg((float) $suggestion['suggested_weight_kg'], $session['exercises'][$index]['weight_unit'] ?? 'kg')) ?>"></label><button class="button button-primary" name="status" value="accepted">Принять</button><button class="button button-secondary" name="status" value="rejected">Отклонить</button></form><?php else: ?><small>Статус: <?= e(system_label('status', $suggestion['status'])) ?><?php if ($suggestion['accepted_weight_kg'] !== null): ?>, принято <?= e(\App\Domain\Weight::fromKg((float) $suggestion['accepted_weight_kg'], $session['exercises'][$index]['weight_unit'] ?? 'kg')) ?> <?= e(unit($session['exercises'][$index]['weight_unit'] ?? 'kg')) ?><?php endif; ?>.</small><?php endif; ?>
    </div>
    <?php endif; ?>
<?php if ($fact['sets']): ?><details class="card edit-completed"><summary>Исправить подходы</summary><?php foreach ($session['exercises'][$index]['sets'] as $set): ?><form method="post" action="<?= e(url('/sets/' . $set['id'] . '/edit')) ?>" class="inline-edit"><input type="hidden" name="_csrf" value="<?= e($csrf) ?>"><input type="hidden" name="session_id" value="<?= (int) $session['id'] ?>"><input type="hidden" name="session_version" value="<?= (int) $session['version'] ?>"><input type="hidden" name="version" value="<?= (int) $set['version'] ?>"><label>Вес <select name="weight_unit" aria-label="Единица веса"><option value="kg" <?= ($set['weight_unit'] ?? 'kg') === 'kg' ? 'selected' : '' ?>><?= e(unit('kg')) ?></option><option value="lb" <?= ($set['weight_unit'] ?? 'kg') === 'lb' ? 'selected' : '' ?>>lb</option></select><input type="number" aria-label="Вес" name="weight_value" min="0" max="2000" step="0.01" value="<?= e($set['weight_value'] ?? $set['weight_kg']) ?>"></label><label>Повторы<input type="number" name="reps" min="1" max="1000" value="<?= (int) $set['reps'] ?>"></label><label>RIR<input type="number" name="rir" min="0" max="10" step="0.5" value="<?= e($set['rir']) ?>"></label><button class="button button-secondary">Сохранить</button></form><form method="post" action="<?= e(url('/sets/'.$set['id'].'/delete')) ?>" class="inline-delete"><input type="hidden" name="_csrf" value="<?= e($csrf) ?>"><input type="hidden" name="session_id" value="<?= (int)$session['id'] ?>"><input type="hidden" name="session_version" value="<?= (int)$session['version'] ?>"><input type="hidden" name="version" value="<?= (int)$set['version'] ?>"><label class="check-row"><input type="checkbox" name="confirm_delete" value="1" required><span>Подтверждаю мягкое удаление</span></label><button class="button button-danger">Удалить подход</button></form><?php endforeach; ?></details><?php endif; ?>
<?php endforeach; ?>
</div></section>
