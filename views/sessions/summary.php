...
$statusLabels = ['completed' => 'Выполнено', 'skipped' => 'Пропущено', 'active' => 'В работе', 'pending' => 'Не начато', 'waiting' => 'Ожидание'];
?>
<?php if ($error): ?><div class="alert alert-error"><?= e($error) ?></div><?php endif; ?>
<?php if ($success): ?><div class="alert alert-success"><?= e($success) ?></div><?php endif; ?>
<section class="summary-hero"><span class="summary-check">✓</span><p class="eyebrow">Тренировка завершена</p><h1><?= e($session['name']) ?></h1><p><?= e(date('d.m.Y', strtotime($session['scheduled_date']))) ?> · <?= (int) $report['session']['duration_minutes'] ?> мин.</p></section>
...
<?php foreach ($report['exercises'] as $index => $exercise): $fact = $exercise['fact']; ?>
    <article class="result-detail">
        <div><strong><?= e($exercise['name']) ?></strong><small><?= e($statusLabels[$fact['status']] ?? $fact['status']) ?> · <?= (int) $fact['working_sets'] ?> раб. · <?= e($fact['tonnage_kg']) ?> кг<?php if ($fact['best_e1rm']): ?> · e1RM <?= e($fact['best_e1rm']['e1rm_kg']) ?> кг<?php endif; ?></small></div>
