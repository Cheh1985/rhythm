...
<?php $lastProgram = null; foreach ($versions as $version): ?>
    ...
    <?php endif; ?>
    <?php if ($version['version_number'] !== null): ?><article class="version-card"><span class="version-badge">v<?= (int) $version['version_number'] ?><?= (int) $version['is_active_version'] === 1 ? ' · '.t('активна') : '' ?></span><div><strong><?= e(local_date($version['version_created']).' '.date('H:i', strtotime($version['version_created']))) ?></strong><p><?= e($version['change_reason']) ?></p><small><?= $version['parent_version'] ? t('Основана на v') . (int) $version['parent_version'] : t('Первая версия') ?> · <?= e((string) $version['lifecycle_status']) ?> · <?= (int) $version['template_count'] ?> <?= t('шабл.') ?> · <?= (int) $version['plan_count'] ?> <?= t('план.') ?></small><?php if ($version['trainer_comment']): ?><small class="trainer-comment"><?= e($version['trainer_comment']) ?></small><?php endif; ?></div></article><?php endif; ?>
<?php endforeach; ?>
</div>
